...
export default function AutosSection({
  ...
  const [testerMatchType, setTesterMatchType] = useState('contains');
  ...
  function runTester(){ 
    ...
    try { 
      ...
      let pattern;
      switch(testerMatchType) {
        case 'exact':
          pattern = `^(?:${testerText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})$`;
          break;
        case 'whole':
          pattern = `\\b(?:${testerText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})\\b`;
          break;
        case 'contains':
        default:
          pattern = testerText.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      ...
      const reg = new RegExp(pattern, testerFlags); 
    ...
    <div className="auto-head mb-3">
      ...
      <div className="auto-head-actions">
        <button className="btn btn-sm btn-regex d-inline-flex align-items-center gap-1" type="button" onClick={()=>setShowRegexTester(s=>!s)}>
          <i className="fa-solid fa-code" /> <span className="d-none d-sm-inline">{showRegexTester? t('autosSection.regexTester.close') : t('autosSection.regexTester.open')}</span>
        </button>
        <button className="btn btn-sm btn-brand d-inline-flex align-items-center gap-1" onClick={openNewAuto} title={t('autoResponses.addResponse')}>
          <i className="fa-solid fa-plus" /> <span className="d-none d-sm-inline">{t('common.add')}</span>
        </button>
      </div>
    </div>
  {showRegexTester && <div className="card card-glass shadow-sm mb-3"><div className="card-body row g-3"><div className="col-md-3"><label className="form-label small mb-1">{t('autosSection.modal.matchType')}</label><select className="form-select form-select-sm" value={testerMatchType} onChange={e=>setTesterMatchType(e.target.value)}><option value="contains">{t('autosSection.modal.matchTypes.contains')}</option><option value="whole">{t('autosSection.modal.matchTypes.whole')}</option><option value="exact">{t('autosSection.modal.matchTypes.exact')}</option></select></div><div className="col-md-3"><label className="form-label small mb-1">{t('autosSection.regexTester.pattern')}</label><input className="form-control form-control-sm" placeholder="hello" value={testerText} onChange={e=>setTesterText(e.target.value)} /></div><div className="col-md-2 col-4"><label className="form-label small mb-1">{t('autosSection.regexTester.flags')}</label><input className="form-control form-control-sm" placeholder="i" value={testerFlags} onChange={e=>setTesterFlags(e.target.value)} /></div><div className="col-md-4"><label className="form-label small mb-1">{t('autosSection.regexTester.sampleLabel')}</label><textarea className="form-control form-control-sm" rows={2} value={testerSample} onChange={e=>setTesterSample(e.target.value)} placeholder={t('autosSection.regexTester.samplePlaceholder')} /></div><div className="col-12 d-flex gap-2"><button className="btn btn-sm btn-brand d-inline-flex align-items-center gap-1" type="button" onClick={runTester}><i className="fa-solid fa-play" /> {t('autosSection.regexTester.run')}</button><button className="btn btn-sm btn-outline-light d-inline-flex align-items-center gap-1" type="button" onClick={()=>{ setTesterMatchType('contains'); setTesterText(''); setTesterSample(''); setTesterResult(null); }}><i className="fa-solid fa-eraser" /> {t('common.clear')}</button></div>{testerResult && <div className="col-12 small">{testerResult.ok ? <div className="vstack gap-1">{testerResult.matches.map((m,i)=><div key={i} className={m.match? 'text-success':'text-muted'}>{m.match? '✓':'✗'} {m.line || <em>({t('autosSection.regexTester.empty')})</em>}</div>)}</div> : <div className="text-danger">{t('autosSection.regexTester.error', { error: testerResult.error })}</div>}</div>}</div></div>}
    ...
    <div className={"bulk-bar mb-2" + (isMobile? ' bulk-bar-mobile-sticky':'')}>
      <div className="d-flex flex-wrap gap-2 align-items-center">
        ...
        {isMobile && <>
          <button className="btn btn-sm btn-outline-light d-inline-flex align-items-center gap-1" onClick={selectAllFiltered} disabled={allFilteredSelected} title={t('autosSection.bulk.selectAllFiltered')}><i className="fa-solid fa-check-double" /> {t('autosSection.bulk.selectAllFiltered')}</button>
          <button className="btn btn-sm btn-enable d-inline-flex align-items-center gap-1" onClick={()=>bulkEnable(false)} disabled={!selectedKeys.size} title={t('autosSection.bulk.enableSelected')}><i className="fa-solid fa-toggle-on" /> {t('autosSection.bulk.enable')}</button>
          <button className="btn btn-sm btn-disable d-inline-flex align-items-center gap-1" onClick={()=>bulkEnable(true)} disabled={!selectedKeys.size} title={t('autosSection.bulk.disableSelected')}><i className="fa-solid fa-toggle-off" /> {t('autosSection.bulk.disable')}</button>
          <button className="btn btn-sm btn-outline-danger d-inline-flex align-items-center gap-1" onClick={bulkDelete} disabled={!selectedKeys.size} title={t('autosSection.bulk.deleteSelected')}><i className="fa-solid fa-trash" /> {t('common.delete')}</button>
          <button className="btn btn-sm btn-outline-secondary d-inline-flex align-items-center gap-1" onClick={clearSelection} disabled={!selectedKeys.size} title={t('autosSection.bulk.clearSelection')}><i className="fa-solid fa-xmark" /> {t('common.clear')}</button>
        </>}
      </div>
