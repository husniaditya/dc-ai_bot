...
// Simple settings panel for per-guild configuration
export default function SettingsPanel({ guildId, pushToast }){
	...
	const zones = useMemo(()=>{
		...
		const computeOffset = (tz) => {
			try {
				...
				// Reconstruct the equivalent UTC timestamp from local wall time in tz
		...
		const items = useMemo(()=>{
				...
				function handleInput(e){
					...
					// If exact match with one of the zones, trigger change
				...
				// Compose display label list for datalist
