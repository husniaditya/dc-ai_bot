      ...
      try {
        if (type === 'toggle') {
          if (hasRole) {
            ...
            console.log(`Added role ${role.name} to ${user.tag} via reaction`);
          }
        } else if (type === 'add_only' && !hasRole) {
          await member.roles.add(role);
          console.log(`Added role ${role.name} to ${user.tag} via reaction (add_only)`);
        } else if (type === 'remove_only' && hasRole) {
          await member.roles.remove(role);
          console.log(`Removed role ${role.name} from ${user.tag} via reaction (remove_only)`);
        }
      } catch (error) {
