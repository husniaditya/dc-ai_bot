    ...
    try {
      const [rows] = await db.sqlPool.query(`
        SELECT clans, clan_name, war_announce_channel_id, member_announce_channel_id, donation_announce_channel_id,
               war_mention_target, member_mention_target, donation_mention_target, enabled, interval_sec, track_wars, track_members, 
               track_donations, donation_threshold, war_start_template, war_end_template, 
               member_join_template, donation_template, embed_enabled, clan_data
...
async function setGuildClashOfClansConfig(guildId, partial) {
  ...
  if (partial.donationThreshold !== undefined) next.donationThreshold = partial.donationThreshold;
  ...
  if (db.mariaAvailable && db.sqlPool) {
    ...
    await db.sqlPool.query(`
      REPLACE INTO guild_clashofclans_watch(
        guild_id, enabled, clans, clan_name, track_wars, track_members, track_donations,
        ...
        embed_enabled, clan_data, interval_sec, donation_threshold
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `, [
      guildId,
      next.enabled ? 1 : 0,
